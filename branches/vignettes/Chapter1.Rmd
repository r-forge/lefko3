...
#### Life table response experiments (LTREs)
  ...
  \begin{equation}
  \tag{20}
  \Delta \lambda = \lambda^m - \lambda^R \approx \sum_{ji} ((a_{ji}^m - a_{ji}^K) (\frac{\partial \lambda}{\partial a_ji}) | _\mathbf{A^{(m+R)/2}})
  \end{equation}
  
  <p style="text-indent: 20px">Since 2010, several stochastic life table response experiment (sLTREs) methodologies have also been developed. sLTREs measure the impact of one or more treatments on the log stochastic growth rate, $a = \text{log} \lambda _{S}$. However, the treatment and reference here are not single matrices, but sets of matrices that vary randomly across time. Here, we assume that $a^{m}$ and $a^{R}$ are the log stochastic growth rates of the treatment MPM and the reference MPM, respectively. Package `lefko3` currently implements the Davison et al. [-@davison_demographic_2010] simulated sLTRE, where:</p>
  
  \begin{equation}
  \tag{21}
  \Delta A = a^{m} - a^{R} \approx \sum_{ji} [log \mu_{ji}^{m} - log \mu_{ji}^{K}] E_{ji}^{\mu} + [log \sigma_{ji}^{m} - log \sigma_{ji}^{K}] E_{ji}^{\sigma}
  \end{equation}
  
In the above equation, $E_{ji}^{\mu}$ and $E_{ji}^{\sigma}$ are the stochastic elasticities of the log stochastic growth rate to changes in the mean and standard deviation, respectively, of the element at row $j$ and column $i$ in the reference matrix set.
