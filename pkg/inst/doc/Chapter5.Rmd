...
#> 1   1     1 -0.04043205 0.002352739 0.04850504 0.0004850504
cypann3s
#>   pop patch           a         var         sd           se
#> 1   1  <NA> -0.03771166 0.001646792 0.04058069 0.0004058069
```
...
![Figure 5.7. Ahistorical vs. historically-corrected deterministic and stochastic reproductive values](figure/Ch5.20-1.png)
...
```
  ...
  <p style="text-indent: 20px">Deterministic sensitivity analysis supported transitions from small non-flowering stages to larger flowering stages, such as 1-sprouted non-flowering adult in occasions *t*-1 and *t* to 10-sprouted flowering adult in occasion *t*+1, as the transitions that $\lambda$ is most sensitive to (as, an example, we can see this by dividing 962,644 by the number of stage pairs, which is 2916 - rounding up the answer gives us the matrix column [331], and the remainder from this division problem gives us the row [364]; the row and column numbers can then be checked against the `hstages` portion of the historical `lefkoMat` object used as input). The stochastic sensitivity analysis gives a slightly different result, supporting the transition from 1-sprouted non-flowering adult in occasions *t*-1 and *t* to 7-sprouted flowering adult in occasion *t*+1. The highest historically-corrected deterministic sensitivity value is associated with the transition from 1-sprouted non-flowering to 10-sprouted flowering (element 364), while the highest historically-corrected stochastic sensitivity value is associated with the transition from 1-sprouted flowering adult to 7-sprouted flowering adult stage (element 1657).</p>
