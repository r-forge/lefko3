...
```
  ...
  <p style="text-indent: 20px">These are giant matrices. With 10,609 rows and columns, there are a total of 112,550,881 elements per matrix. But they are also amazingly sparse - with 915,632 elements estimated, only 0.8% of elements per matrix are non-zero. The survival probability sums all look good, so we appear to have no problems with overly large given and proxy survival transitions provided through our supplemental tables. Let's now build the ahistorical IPMs.</p>
...
```
  ...
  <p style="text-indent: 20px">The ahistorical IPMs are certainly smaller than the historical IPMs, but are nonetheless quiet large. Although huge, these matrices are not sparse - 9,187 elements out of 10,609 per matrix are estimated (86.6%).</p>
...
#> 1   1     1 -0.2092219 5.944312e-24 2.438096e-12 2.438096e-14
set.seed(42)
slambda3(lathmat3ipm, times = 1000)
#>   pop patch           a          var           sd           se
#> 1   1     1 -0.07880318 6.380504e-20 2.525966e-10 7.987806e-12
```
...
![Figure 6.7. Ahistorical vs. historically-corrected elasticity of lambda to stage](Ch6.21-1.png)

```r
par(lty = lty.o)
```
  
  <p style="text-indent: 20px">The plot of these distributions shows the strong importance of dormant seeds, vegetative dormancy, and small adults. The tallest bars in ahistorical analyses correspond to dormant adults. In contrast, historical analyses both show dormant seeds as most important, followed by small adults and only later dormant individuals. All three stages are high in all analyses, though.</p>
...
![Figure 6.8. Ahistorical vs. historically-corrected elasticity of lambda to transitions](Ch6.22-1.png)
  
  <p style="text-indent: 20px">We see similar transition elasticities between ahistorical and historical analyses, with growth the most influential on $\lambda$, while stasis or fecundity is the least influential depending on whether the analysis is ahistorical or historical.</p>
