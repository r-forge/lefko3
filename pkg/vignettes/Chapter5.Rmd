...
![Figure 5.2. Histogram of Cypripedium candidum size and fecundity in occasion t](Ch5.3-1.png)
  ...
  <p style="text-indent: 20px">Stem number has a very low mean, and the curve declines fairly smoothly. The Poisson or negative binomial distribution might work for size. Fecundity appears to have many 0s, suggesting that we might need a zero-inflated count distribution.</p>
...
#### Step 3c. Model building and selection
  ...
  <p style="text-indent: 20px">The mixed modeling approach is usually preferable to the GLM approach. However, a mixed modeling strategy results in lower statistical power and a greater time used in estimating models (or, conversely, it yields truer statistical power while the GLM approach inflates Type I error). Greater complexity may also make failed model selection more likely. Users of package `lefko3` wishing to use a standard generalized linear modeling strategy can set `approach = "glm"`. In this case, individual identity is not used, time is a fixed factor, and all observed transitions are treated as independent.</p>
  
  <p style="text-indent: 20px">Next, we must determine which vital rates to model. Function `modelsearch()` estimates up to 13 vital rate models:</p>
  ...
  10) juvenile primary size in occasion *t*+1 assuming survival and observation in that time, and
  
  11) juvenile secondary size in occasion *t*+1 assuming survival and observation in that time, and
...
#> Hessian with 2 negative eigenvalues
...
#> This lefkoMat object covers 1 populations, 1 patches, and 6 time steps.
#> 
...
#> Juvenile survival estimated with 70 individuals and 288 individual transitions.
#> Juvenile observation estimated with 51 individuals and 118 individual transitions.
...
![Figure 5.4. Image of ahistorical element-wise matrix mean](Ch5.16-1.png)
...
![Figure 5.5. Images of historical element-wise matrix mean](Ch5.17-1.png)
...
![Figure 5.6. Ahistorical vs. historical annual population growth rate](Ch5.20-1.png)
...
![Figure 5.7. Ahistorical vs. historically-corrected stable and long-run mean stage distribution](Ch5.21-1.png)
...
![Figure 5.8. Ahistorical vs. historically-corrected deterministic and stochastic reproductive values](Ch5.22-1.png)
...
![Figure 5.9. Ahistorical vs. historically-corrected deterministic and stochastic elasticity to stage](Ch5.26-1.png)
...
![Figure 5.10. Ahistorical vs. historically-corrected elasticity of lambda to transitions](Ch5.27-1.png)
...
![Figure 5.11. Elasticity of lambda to historical transitions](Ch5.28-1.png)
