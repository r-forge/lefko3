...
```r
sf_distrib(vertdata, sizea = c("size3added", "size2added"), obs3 = "obsstatus3",
...
#### Step 2c. Provide supplemental information for matrix estimation  
  ...
  <p style="text-indent: 20px">We will create two supplement tables, the first covering the historical analysis, and the second covering the ahistorical analysis. Each row refers to a specific transition, and in the historical case, there are codes for 19 given transitions (12 for the ahistorical case). The first nine of the historical transitions are set to specific probabilities, and the next eight are transitions that will be set to other, estimated transitions acting as proxies (these are the non-NA transitions in `eststage` set below). The final two terms are fecundity multipliers. Based on the literature, the proxies for entry into the adult classes are transitions from dormancy, as below. Where necessary, we also use `rep` and `mat` as shorthand to code for all reproductive stages and all mature stages, respectively.</p>
...
#### Step 3c. Model building and selection  
  ...
  12) juvenile tertiary size in occasion *t*+1 assuming survival and observation in that time, and
  
  13) reproduction status in occasion *t*+1 assuming survival and observation until that time of a juvenile in occasion *t* that is becoming mature in occasion *t*+1.
...
```
  ...
  <p style="text-indent: 20px">We see many more total elements (over 8.5 million, in comparison to 2,916 in the ahistorical case), and many more rows and columns (54 rows and columns in the ahistorical case, and 54^2 = 2,916 rows and columns in the historical case). However, historical matrices are composed primarily of structural zeros. Thus, with only 117,712 + 2,352 = 120,064 elements estimated per matrix, only 1.4% of elements are non-zero (the equivalent percentage for the ahistorical case is 84.3%).</p>
...
![Figure 5.3. Image of ahistorical element-wise matrix mean](Ch5.15-1.png)
...
![Figure 5.4. Images of historical element-wise matrix mean](Ch5.16-1.png)
...
![Figure 5.7. Ahistorical vs. historically-corrected deterministic and stochastic reproductive values](Ch5.21-1.png)
  ...
  <p style="text-indent: 20px">We see some big differences here. In the deterministic analyses, the historical case predicts a greater importance of small and medium-sized flowering adults, and a lower importance of all other stages than the ahistorical case. In the stochastic analyses, the highest reproductive values are in the largest flowering adults, though large non-flowering and medium-sized adults have reproductive values almost as high. So, both history and temporal stochasticity have large effects here. Interesting results in need of further study!</p>
...
#> 
#> The highest deterministic sensitivity value: 0.21326673282483
...
#> 
#> The highest stochastic sensitivity value: 0.221340542385381
writeLines(paste0("This value is associated with element: ",
...
#> The highest deterministic sensitivity value: 0.0571803480842823
writeLines(paste0("This value is associated with element: ",
...
#> The highest stochastic sensitivity value: 0.0590004698453227
writeLines(paste0("This value is associated with element: ",
...
#> The highest deterministic sensitivity value (historically-corrected): 0.216067454248667
writeLines(paste0("This value is associated with element: ",
  which(tm3sens$ah_sensmats[[1]] ==
    max(tm3sens$ah_sensmats[[1]][which(tmeans2r$A[[1]] > 0)]))))
#> This value is associated with element: 374
...
#> The highest stochastic sensitivity value (historically-corrected): 0.225477908733132
writeLines(paste0("This value is associated with element: ",
...
![Figure 5.8. Ahistorical vs. historically-corrected deterministic and stochastic elasticity to stage](Ch5.25-1.png)
  ...
  <p style="text-indent: 20px">Elasticity analyses in these plots look fairly similar. Population growth rates are most elastic in response to changes in stage 7, which is the one-sprouted non-flowering adult stage. Generally speaking, small adult stages have the highest elasticity. Most of the differences between analyses here appear to lie in elasticity of small non-flowering vs. flowering adults in the ahistorical vs. historical comparison.</p>
...
![Figure 5.9. Ahistorical vs. historically-corrected elasticity of lambda to transitions](Ch5.26-1.png)
  ...
  <p style="text-indent: 20px">Package `lefko3` also includes functions to conduct deterministic and stochastic life table response experiments, and a general projection function that can be used for analyses such as quasi-extinction analysis as well as for density dependent analysis. Users wishing to conduct these analyses should see our free e-manual called ***lefko3: a gentle introduction*** and other vignettes, including long-format and video vignettes, on <a href = "http://www.sheffersonlab.com/projects">the projects page</a> of the Shefferson lab website.
