...
### BASIC WORKFLOW
  ...
  <p style="text-indent: 20px">First, we will clear memory, and load the package and the dataset.</p>
...
#> Length of fecundity variable in t+1: 2552, and # non-integer entries: 0
...
#> Length of fecundity variable in t: 2552, and # non-integer entries: 6
...
#> Length of fecundity variable in t-1: 2552, and # non-integer entries: 6
```
...
```
  ...
  <p style="text-indent: 20px">Such significant results for both fecundity distribution tests show us that we really need to use a zero-inflated negative binomial distribution.</p>
...
```
  ...
  <p style="text-indent: 20px">Next, we will estimate the reproductive values associated with the element-wise mean matrices. Note that we are standardizing by the maximum value in each case in order to fit a;; four sets into the sam graph. Normally, we would not do this in inference, but it can help a quick visual comparison.</p>
